document.getElementById('form_acceptance_terms').addEventListener('submit', function (event) {
    ...
    // gera_log('Acceptance Terms', 'Clicou para confirmar o termo de aceite.');
    ...
    fetch('./accept_terms', options).then(function (response) {
        ...
        response.json().then(function (jsonResponse) {
            if (jsonResponse.msg === 'success') {
                gera_log('Acceptance Terms', "Termo de aceite confirmado com sucesso.");
                setTimeout(() => {
                    window.location.replace('/');
                }, 1000);
            } else {
                gera_log("Termo de Aceite", "Erro ao confirmar aceite de termo.");
                modalMessage({
                    title: 'Acceptance Terms',
